...
設定は独立した「MCP接続」タブに置き、開発中のファイル文字起こし専用機能として表示します。
...
録音中の補正が無効なら停止時に補正します。ファイル読込・再認識・MCPは従来のローカル経路です。
APIのタイムアウト・切断・利用上限は明示し、認識済み原文を保持します。課金APIへ自動再試行しません。
