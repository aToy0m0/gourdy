...
現在の配布版は **0.7.63（プレリリース）** です。
[GitHub Releases](https://github.com/aToy0m0/gourdy/releases/tag/v0.7.63) でWindows x64用ZIP、インストーラーと対応ソースを配布しています。

## 配布物

- `gourdy-0.7.63-windows-x64-setup.exe`：ユーザー単位のインストーラー。実行環境を同梱し、スタートメニューへ登録。
- `gourdy-0.7.63-windows-x64.zip`：アプリ、依存ランタイム、公開ドキュメント、ライセンス。
- `gourdy-0.7.63-corresponding-sources.zip`：FFmpeg、Moonshine内Eigen、ONNX Runtime用Eigen、certifi、tqdmの対応ソース、FFmpegビルド手順と取得物のハッシュ。
...
npm run release:zip
Expand-Archive release/gourdy-0.7.63-windows-x64.zip -DestinationPath dist-installer-input
