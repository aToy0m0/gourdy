...
function validate(settings) {
  ...
  if (!colors.includes(settings.accent) || ['closeToTray','textBackground','liveInput','showTaskbar','launchAtStartup','imeAutoImport','saveAudio','commandEnabled','noiseSuppression','continuationAssist','fastStart'].some(k => typeof settings[k] !== 'boolean')) throw new Error('操作設定が不正です。');
