...
const defaults = {
  shortcut: 'Super+J', commandShortcut: 'Super+Shift+J', commandEnabled: false, saveAudio: true, replacements: [], liveInput: true,
  microphoneId: '', noiseSuppression: true,
...
function validate(settings) {
  if (!settings || typeof settings.shortcut !== 'string' || settings.shortcut.length > 80 || !/^(?=.*(?:Control|CommandOrControl|Alt|Super)).+\+[^+]+$/.test(settings.shortcut)) throw new Error('Ctrl・Alt・Winのいずれかを含むショートカットを指定してください。');
  ...
  if (!colors.includes(settings.accent) || ['liveInput','showTaskbar','launchAtStartup','imeAutoImport','saveAudio','commandEnabled','noiseSuppression'].some(k => typeof settings[k] !== 'boolean')) throw new Error('操作設定が不正です。');
