...
class RecordingShortcut {
 constructor(globalShortcut,callback,onError,hold=false){this.hold=hold;this.global=globalShortcut;this.callback=callback;this.onError=onError;this.helpers=new Map();}
 async register(key){
  if(!singleKeys[key])return this.global.register(key,this.callback);
  const child=spawn(path.join(__dirname,'ModifierShortcut.exe'),[String(singleKeys[key]),...(this.hold?['hold']:[])],{windowsHide:true,stdio:['pipe','pipe','pipe']});let error='',ready=false,closed=false;
  ...
  return new Promise((resolve,reject)=>{
   ...
   createInterface({input:child.stdout}).on('line',line=>{if(line==='ready'){ready=true;clearTimeout(timer);this.helpers.set(key,stop);resolve(true);}else if(line===(this.hold?'press':'tap')&&ready&&!closed)this.callback();});
