...
class RealtimeInput {
  constructor(target,signal){
    ...
    createInterface({input:this.child.stdout}).on('line',line=>{
      try{const response=JSON.parse(line),pending=this.pending;if(response.ready)this.verification=response.verification;if(!pending)throw new Error('入力監視の応答順が不正です。');this.pending=null;clearTimeout(pending.timer);if(Object.hasOwn(response,'error'))pending.reject(new Error(response.error||'入力監視で詳細不明のエラーが発生しました。'));else if(response.ready===true||typeof response.written==='string')pending.resolve(response);else pending.reject(new Error('入力監視の応答形式が不正です。'));}
