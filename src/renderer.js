...
for(let i=0;i<20;i++)$('wave').append(document.createElement('i'));
function configure(value){settings=value;document.documentElement.style.setProperty('--accent',value.accent);document.documentElement.style.setProperty('--icon-active-background',value.accent==='#262626'?'#dedede':value.accent);render()}
function render(){const active=phase==='recording';$('record').classList.toggle('active',active);$('record').disabled=restarting;$('record').setAttribute('aria-label',active?'録音を停止':phase==='processing'||phase==='starting'?'中断して録音を再開':'録音を開始');$('record').title=$('record').getAttribute('aria-label');document.body.classList.toggle('error',Boolean(notice));
  if(notice){$('status').textContent=notice.includes('Esc')?'入力先をクリック · Escで中止':notice.split(/[。\n]/)[0];$('status').title=notice;$('settings').title=notice;return;}
  $('status').title='';$('settings').title='設定';if(outcome&&phase==='idle'){$('status').textContent=outcome;return;}$('status').textContent=active?(command?'コマンド · キーを離して実行':'録音中 00:00'):phase==='starting'?'マイクを準備中':phase==='processing'?'補正中 · クリックで再開':`クリックで録音開始\nor ${(settings?.shortcut||'').replace('CommandOrControl','Ctrl').replace('Super','Win').split('+').join(' + ')}`;
}
...
const levels=Array(20).fill(0);
function resetWave(){levels.fill(0);for(const bar of $('wave').children){bar.style.height='3px';bar.style.background='#b8b8b8'}}
function audio(pcm){levels.shift();levels.push(Microphone.level(pcm));[...$('wave').children].forEach((bar,i)=>{bar.style.height=(3+levels[i]*23)+'px';bar.style.background='var(--accent)'});if(failure||cancelled)return;
  ...
  pending.push(pcm);pumpAudio();
}
function pumpAudio(){if(pump||!backendReady)return;
...
async function toggle(mode='dictation'){
  ...
  if(phase!=='idle'&&(notice||phase==='starting'||phase==='processing')){
...
api.on('command-start',()=>toggle('command').catch(error));api.on('command-release',value=>{released=true;if(value.error)error(new Error(value.error));if(command&&phase==='recording')trackStop().catch(error);});api.on('command-progress',value=>{if(phase==='processing')$('status').textContent=value+' · クリックで中止';});api.on('command-result',value=>{outcome=value;});$('settings').onclick=()=>api.openSettings(notice?'history':'operation').catch(error);$('hide').onclick=()=>api.hide().catch(error);
