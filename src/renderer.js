...
function configure(value){settings=value;if(value.inputReady)setupNotice='';$('hide').title=value.closeToTray?'トレイへ閉じる':'アプリを終了';const mono=value.accent==='#262626',base=mono?'#e4e4e4':value.accent,dark=mono?'#a0a0a0':'#'+base.slice(1).match(/../g).map(x=>Math.round(parseInt(x,16)*.72).toString(16).padStart(2,'0')).join('');document.documentElement.style.setProperty('--accent',value.accent);document.documentElement.style.setProperty('--icon-background',base);document.documentElement.style.setProperty('--icon-active-background',dark);document.documentElement.style.setProperty('--wave-background',mono?'#777':base);document.documentElement.style.setProperty('--wave-active-background',mono?'#444':dark);document.body.classList.toggle('text-background',value.textBackground!==false);render()}
...
let infoMessage='',dismissedNotice='',setupNotice='';
api.on('setup-needed',text=>{setupNotice=text;dismissedNotice='';render();});
function syncInfo(){
  const current=notice||setupNotice;const message=current!==dismissedNotice?current:'';
