...
contextBridge.exposeInMainWorld('dictation',{
  ...
  on:(channel,callback)=>{if(!['continuation','command-model-changed','command-progress','command-start','command-release','command-result','toggle','notice','settings-changed','data-changed','request-close','select-tab','phase-changed','meeting-changed'].includes(channel))throw new Error('Unknown event');ipcRenderer.on(channel,(_,value)=>callback(value));}
});
