...
吹き出しに残った文章は、補正中でもコピーボタンやCtrl+Cでコピーできます。補正終了後は、入力先を選んで「入力」から送れます。範囲を選択すると、その部分だけをコピー・入力できます。
ショートカットのダブルタップで録音画面を表示・格納できます。
録音画面の白い外枠をドラッグすると移動し、右下にホバー表示される三日月をドラッグするとサイズを変えられます。
...
| AIツールからファイルを処理する | ファイル文字起こし・辞書編集・履歴確認のMCP接続（開発中）。設定から無効化可能 |
